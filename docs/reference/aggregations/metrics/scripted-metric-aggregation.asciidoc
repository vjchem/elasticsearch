...
Whilst any valid script object can be used within a single script, the object that the scripts return or store in the `_agg`  must be one of the following types:
